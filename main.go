...
func main() {
	...
	for update := range updates {
		...
		if update.Message.NewChatMembers != nil && len(*update.Message.NewChatMembers) != 0 {
			...
			msg += "!\n"
			msg += fmt.Sprintf(
				"Просим Вас [зарегестрироваться](t.me/%s?start=reg) в боте, чтобы Вы получали уведомления от @all", bot.Self.UserName)
		...
		if startsList(txt, []string{"/reg", ("/reg@" + bot.Self.UserName), "/start reg",
			...
			if isInRegs(from) {
				reply(mid, "Вы уже зарегестрировались", peer, bot)
				continue
			}
			regs = append(regs, from)
			reply(mid, "Теперь вы зарегестрированны!", peer, bot)
			write()
			...
			if isInRegs(from) {
				...
				reply(mid, "Вы не зарегестрированны...", peer, bot)
			}
			...
			for _, word := range strings.Fields(txt) {
				...
				case "@all":
					...
					if msg == "all:\n" {
						reply(mid, fmt.Sprintf(
							"all: Никто в этом чате не зарегестрирован...\n\n/reg@%s для регистрации",
